...
City of Vancouver Archives Enduro workflows.
...
This project is configured by default to be able to run a child workflow
...
persistent volume claim called `cva-enduro-workflows-pvc` that will be mounted
in Enduro's a3m or Archivematica worker to be able to share the filesystem with
the cva-enduro worker. This solution will only work in single node
Kubernetes clusters.
...
The child workers need to share the filesystem with Enduro's a3m or
...
```bash
k3d cluster create cva-enduro --registry-create sdps-registry
```

Or using an existing registry:

```bash
k3d cluster create cva-enduro --registry-use sdps-registry
```
...
```bash
git clone git@github.com:artefactual-sdps/cva-enduro-workflows.git
cd cva-enduro-workflows
```
...
```bash
k3d cluster stop cva-enduro
```

To start the environment again:

```bash
k3d cluster start cva-enduro
tilt up
```
...
Enables live updates on code changes for the worker.
...
cva-enduro workflow. Click the caret to set the path to a file/directory in
